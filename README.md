...
- **Edge labels by relationship type:** a link ontology in `static/entities.js` decides which relationships write their name on the canvas. A label is hidden where the target node's own type already states it and the edges fan out — twenty red CVE diamonds do not each need "vulnerable_to" beside them — and shown where the node alone is ambiguous (a port circle reading `5000`) or the claim is unusual (`same_machine`). Labels outside the ontology always show, so hand-drawn graphs are unaffected. **View → Edge labels** switches between by-type / always / never, and lists the labels present in the current graph for per-type overrides; those ride with the graph in `layoutSettings`, so an export reads the way its author arranged it.
